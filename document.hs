...
example4 =
    ...
    , Paragraph (
            "Now, we can compile the program by invoking"
            <> "ghc with the file name:")
    , CodeBlock
        [ "➜ ghc hello.hs"
    ...
    , Paragraph (
        "GHC will produce an executable when the source"
        <> "file satisfies both conditions:"
        )
    , OrderedList
...
main :: IO ()
main = putStrLn "Document module loaded."
